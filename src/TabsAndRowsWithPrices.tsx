...
import type { MenuData, Promo, MenuItem } from "./types";
...
const MenuItem = ({ item }: { item: MenuItem; currency: Currency }) => {
  return (
...
export default function TabsAndRowsWithPrices() {
        ...
        <div className="space-y-4">
          {itemsToShow.length > 0 ? (
            itemsToShow.map((item) => (
              <MenuItem key={item.id} item={item} currency={menu.currency} />
            ))
          ) : (
